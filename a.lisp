 ...
 (let* ( (num (simplify cnum))
  ...
  (if (>= num-deg den-deg)
      (let* (
        ...
        (if (= 0 (remainder div)) (list (list (quotient div) 1))   (append (list (list (quotient div) 1))  (split-frac (remainder div) den)) )
...
(defun remove-constant (factor-list)
  ...
  (let ((f (first factor-list)))
      (if (= 0 (polynomial-degree (second f))) (remove-constant (cdr factor-list))   (append (list f) (remove-constant (cdr factor-list))))
  ))
)
